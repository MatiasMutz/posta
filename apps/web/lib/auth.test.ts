...
describe('rutaInicioPorRol', () => {
  it('devuelve la ruta de inicio según rol', () => {
    expect(rutaInicioPorRol('dueno')).toBe('/dashboard');
    expect(rutaInicioPorRol('vendedor')).toBe('/ventas');
    expect(rutaInicioPorRol('contador')).toBe('/contador');
...
describe('redirectPorRol', () => {
  it('bloquea importar y configuración para no-dueño', () => {
    ...
    expect(redirectPorRol('/importar/job-1', 'contador')).toBe('/contador');
    expect(redirectPorRol('/configuracion/equipo', 'vendedor')).toBe('/ventas');
    expect(redirectPorRol('/importar', 'dueno')).toBeNull();
  });

  it('bloquea dashboard a no-dueño', () => {
    expect(redirectPorRol('/dashboard', 'vendedor')).toBe('/ventas');
    expect(redirectPorRol('/dashboard', 'contador')).toBe('/contador');
    expect(redirectPorRol('/dashboard', 'dueno')).toBeNull();
  });

  it('bloquea contador al vendedor', () => {
    expect(redirectPorRol('/contador', 'vendedor')).toBe('/ventas');
    expect(redirectPorRol('/contador', 'contador')).toBeNull();
    expect(redirectPorRol('/contador', 'dueno')).toBeNull();
  });
  ...
  it('bloquea inventario al contador y movimientos al vendedor', () => {
    expect(redirectPorRol('/inventario', 'contador')).toBe('/contador');
  ...
  it('bloquea POS al contador e historial al vendedor', () => {
    expect(redirectPorRol('/ventas', 'contador')).toBe('/contador');
